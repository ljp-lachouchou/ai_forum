...
本项目致力于为用户提供一个简洁、高效的社区交流平台。通过采用最新的 Android 开发技术栈和 Clean Architecture 架构设计，AIForum 具备良好的可维护性和扩展性。应用采用离线优先策略，优先从本地数据库读取数据，通过 WorkManager 在后台自动同步远程数据，确保网络不稳定时仍能正常使用。同时支持实时推送通知、个性化心情主题等特色功能，为用户带来流畅的使用体验。
...
- **Paging 3**
  - 分页数据加载解决方案
  - 支持无限滚动和列表分页
  - RemoteMediator 支持离线优先策略，协调本地与远程数据
  - 内置缓存机制，提升用户体验
...
- 📨 **推送通知** - 极光推送支持
- 📝 **Markdown 支持** - 富文本内容
- 📴 **离线优先** - 优先读取本地数据，WorkManager 后台自动同步
...
┌──────────────────────┴──────────────────────────────────┐
│                 同步层 (Sync / Worker)                   │
│     WorkManager + RemoteMediator + 离线优先策略          │
└─────────────────────────────────────────────────────────┘
```
...
- 处理数据转换和映射
- 实现缓存策略，优化性能
- **离线优先策略**：优先读取本地数据，WorkManager 后台同步

### 数据流向

项目采用**离线优先策略**，优先从本地数据库读取数据，WorkManager 在后台自动同步远程数据：

```
用户操作 → UI → ViewModel → Use Case → Repository
                                            │
                                            ├─→ 本地数据库 (Room) [优先]
                                            │         ↓
                                            │    立即返回数据
...
5. **离线优先 (Offline-First)**
   ...
   - Paging 3 RemoteMediator 协调本地与远程数据源
   - 网络不稳定时用户仍可正常使用
